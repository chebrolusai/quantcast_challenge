...
for eachFile in [dParsedArguments.get('fileSpec')]:
    ...
    with open(eachFile, READ_ONLY_CONSTANT) as file:
        ...
        # Since the results are sorted, we can stop searching once the date we wanted passed
        dateMatched = False

        for line in file:

            # ignore line if empty or if there arent exactly 2 comma separated values
            if not line.strip():
